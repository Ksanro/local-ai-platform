    ...
    async def execute(self, context: PipelineContext) -> PipelineStageResult:
        ...
        try:
            ...
            if self._delta_enabled and candidates:
                ...
                # Store the union under the STORE key (hash of all user
                # messages incl. the current one). Next turn's conversation_key
                # lookup computes this same value, so the union is found then.
