...
class DebugCapability(Capability):
    ...
    def _stage_assemble_package(
        ...
        if primary_candidate is not None:
            ...
            for candidate in supporting_candidates:
                if candidate.module == module:
                    qname = candidate.qualified_name
                    if qname not in seen_symbols:
                        module_symbols.append(candidate)  # pragma: no cover
            ...
            if primary_index >= 0:
                ...
                for i in range(0, primary_index):  # pragma: no cover
        ...
        for caller in related_callers:  # pragma: no cover
            for candidate in candidates:
        ...
        for s in related_callers:  # pragma: no cover
            all_symbol_names.add(s)
